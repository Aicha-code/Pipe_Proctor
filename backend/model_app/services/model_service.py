...
DEFAULT_MODEL_PATHS = [
    os.path.join(BASE_DIR, "models", "CROMA_base.pt"),
    os.path.join(BASE_DIR, "pretrained_model", "CROMA_base.pt"),
    os.path.join(BASE_DIR, "data", "models", "CROMA_base.pt"),
]
MODEL_PATH = os.getenv("CROMA_MODEL_PATH")
if MODEL_PATH is None:
    MODEL_PATH = next((path for path in DEFAULT_MODEL_PATHS if os.path.exists(path)), DEFAULT_MODEL_PATHS[0])
...
def _get_model():
    ...
    try:
        from model_app.use_croma import PretrainedCROMA
    except ImportError:
        try:
